 ...
 */
export async function loadPersistedState(projectDir: string): Promise<Map<string, PersistedFileState>> {
 ...
 */
export async function savePersistedState(projectDir: string, state: Map<string, PersistedFileState>): Promise<void> {
    const statePath = path.join(projectDir, STATE_FILE_NAME)
...
export function validatePersistedTimestamp(
    ...
    if (currentHash === persistedState.contentHash) {
        debug(`Hash matches for persisted state - trusting timestamp ${persistedState.timestamp}`)
        return persistedState.timestamp
    }

    debug("Hash mismatch for persisted state - file was edited while CLI was offline")
    return null
}
