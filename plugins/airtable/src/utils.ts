...
/**
 * Post-processes HTML generated by marked.parse() to handle edge cases and
 * ensure consistent formatting. While marked handles most markdown conversion,
 * this function applies additional regex replacements to:
 * - Ensure all heading levels are properly converted
 * - Handle blockquotes that may not be fully processed
 * - Apply text formatting (bold, italic, strikethrough)
 * - Process inline code and code blocks
 * - Convert markdown links to HTML anchors
 * - Properly structure lists and paragraphs
 *
 * This is necessary because marked's output may not always match our exact
 * formatting requirements, especially for complex nested structures.
 */
function refineMarkdownHTML(value: string) {
    let html = value
...
export function richTextToHTML(cellValue: string): string {
    const markdownToHTML = marked.parse(cellValue, {
    ...
    return refineMarkdownHTML(markdownToHTML)
}
