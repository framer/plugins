...
export function App() {
  ...
  const command =
    state.project &&
    `npx framer-code-link ${shortProjectHash(state.project.id)}`
  ...
  switch (state.mode) {
    ...
    default:
      framer.setBackgroundMessage(backgroundStatusFromMode(state?.mode))
      framer.hideUI()
      return null
  }
}
    ...
    <main>
      <div className="info">
        ...
        <p>
          ...
          <a
            href="https://github.com/framer/plugins"
            target="_blank"
      ...
      <div className="command-container">
        ...
        <button
          className="framer-button-primary copy-button"
          onClick={handleCopy}
        >
          {copyState === "copied" ? (
            <>
            ...
            <span
              className={copyState === "returning" ? "copy-label-animate" : ""}
            >
              Copy
            </span>
          )}
...
function DeletePanel({ files, onConfirm, onKeep }: DeletePanelProps) {
  ...
  const text = multiple
    ...
    : {
        ...
        description:
          "Code file was deleted locally and will be permanently removed from this Project.",
      }
  ...
  return (
    <main className="user-action-view delete-confirmation">
      ...
      <div className="actions">
        ...
        <button
          className="framer-button-primary destructive-button"
          onClick={onConfirm}
        >
          Delete {multiple ? "All" : ""}
        </button>
  ...
  return (
    <main className="user-action-view">
      ...
      <header>
        ...
        <p>
          The following code files have changed in Framer and locally. Select
    ...
    switch (message.type) {
      ...
      case "file-delete":
        if (message.requireConfirmation) {
          log.debug(
            `Delete requires confirmation for ${message.fileNames.length} file(s)`
          )
      ...
      case "conflicts-detected":
        log.debug(
          `Received ${message.conflicts?.length ?? 0} conflicts from CLI`
        )
        dispatch({ type: "conflicts", conflicts: message.conflicts ?? [] })
        break
      case "conflict-version-request": {
        log.debug(
          `Fetching conflict versions for ${message.conflicts?.length ?? 0} files`
        )
        const versions = await api.fetchConflictVersions(
          message.conflicts ?? []
        )
