...
): { xliff: Document; targetLocale: Locale } {
...
function statusToXliffState(
	status: LocalizedValueStatus,
): "new" | "needs-translation" | "translated" | "signed-off" {
	switch (status) {
		case "new":
			return "new";
		case "needsReview":
...
export function generateSourceXliff(
...
async function checkAndCreateLanguage(
	projectId: number,
	language: Locale,
	accessToken: string,
): Promise<void> {
...
export async function getProjectTargetLanguageIds(
	projectId: number,
	accessToken: string,
): Promise<string[]> {
...
export async function getStorageId(fileName: string, accessToken: string): Promise<number> {
	try {
		...
		const existingStorage = parsed.output.data.find((item) =>
			item.data?.fileName?.includes(fileName),
		);
