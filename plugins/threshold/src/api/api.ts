...
export class PluginApi implements API {
    ...
    async addImage(image: PluginImageInput, options?: ImageInputOptions): Promise<void> {
        const data = await createImageDataFromInput(image)

        return this.invokeTransferable("addImage", [data.bytes.buffer], data, options)
    }

    async uploadImage(image: PluginImageInput, options?: ImageInputOptions): Promise<PluginImage> {
        const data = await createImageDataFromInput(image)

        const imageData = await this.invokeTransferable("uploadImage", [data.bytes], data, options)

        return new PluginImage(imageData, this)
    }

    async getImageData(image: PluginImageIdentifier): Promise<ImageDataTransfer | null> {
...
export interface API {
    ...
    addImage: (image: PluginImageInput, options?: ImageInputOptions) => Promise<void>
    /** Upload an image without assigning it. */
    uploadImage: (image: PluginImageInput, options?: ImageInputOptions) => Promise<PluginImage>

    /**
     * Get the details of the given image input.
     */
    getImageData: (image: PluginImage) => Promise<ImageDataTransfer | null>
...
export interface PluginMessageAPI extends Pick<API, "closeWindow" | "closePlugin" | "getCurrentUser" | "addSVG"> {
    showWindow: (options: WindowOptions) => Promise<void>
    ...
    addImage: (image: ImageDataTransfer, options: ImageInputOptions | undefined) => Promise<void>

    uploadImage: (image: ImageDataTransfer, options: ImageInputOptions | undefined) => Promise<PluginImageData>

    getImageData: (image: PluginImageIdentifier) => Promise<ImageDataTransfer>
