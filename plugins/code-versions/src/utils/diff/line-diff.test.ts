...
describe("divider generation", () => {
    it("generates dividers when changes are far apart", () => {
        ...
        expect(dividers[0]?.line).toEqual(1)
    })
...
describe("getLineDiffWithEdges", () => {
    ...
    it("sets correct edges when a change block follows a change block", () => {
        ...
        expect(changes[0]?.removeIsTopEdge).toBe(true)
        expect(changes[0]?.removeIsBottomEdge).toBe(false)
        expect(changes[1]?.addIsTopEdge).toBe(false)
        expect(changes[1]?.addIsBottomEdge).toBe(true)
    })
})
