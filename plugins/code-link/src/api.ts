...
export class CodeFilesAPI {
  ...
  async fetchConflictVersions(
    requests: Array<{ fileName: string; lastSyncedAt?: number }>
  ) {
    ...
    const versionPromises = requests.map(async (request) => {
      ...
      const file = codeFiles.find(
        (f) =>
          canonicalFileName(f.path || f.name) ===
          canonicalFileName(request.fileName)
      )
      ...
      try {
        ...
        if (versions && versions.length > 0) {
          ...
          log.debug(
            `${request.fileName}: ${versions[0].createdAt} (${latestRemoteVersionMs})`
          )
          return {
            fileName: request.fileName,
...
async function upsertFramerFile(
  fileName: string,
  content: string
  ...
  const existing = codeFiles.find(
    (file) =>
      canonicalFileName(file.path ?? file.name) === canonicalFileName(fileName)
  )
...
async function deleteFramerFile(fileName: string) {
  ...
  const codeFiles = await framer.getCodeFiles()
  const existing = codeFiles.find(
    (file) => canonicalFileName(file.path ?? file.name) === normalizedTarget
  )
