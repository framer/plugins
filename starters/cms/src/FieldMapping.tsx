...
import { type DataSource, mergeFieldsWithExistingFields, syncCollection, useDataSourceHeaders } from "./data"
...
export function FieldMapping({ collection, dataSource, initialSlugFieldId }: FieldMappingProps) {
    ...
    const { dataSourceHeaders } = useDataSourceHeaders()
    const dataSourceName = dataSourceHeaders.find(header => header.id === dataSource.id)?.name ?? dataSource.id
