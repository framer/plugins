...
function getRandomizeRectsForCanvas(
  ...
  for (const rect of rects) {
    ...
    for (let i = 0; i < maxTries; i++) {
      ...
      const isOverlapping = placedRects.some((rect) =>
        rectsIntersect(rect, randomRectWithOffset),
      );
...
function getRandomizedRects(rects: RectWithId[], gap: number): RectWithId[] {
  ...
  while (true) {
    const randomRects = getRandomizeRectsForCanvas(
      rects,
      gap,
      canvasWidth,
      canvasHeight,
...
export function App() {
  ...
  const [layout, setLayout] = useLocaleStorageState<Layout>(
    "layout",
    "horizontal",
    isLayout,
  );

  const [sorting, setSorting] = useLocaleStorageState<Sorting>(
    "sorting",
    "position",
    isSorting,
  );

  const [columnCount, setColumnCount] = useLocaleStorageState(
    "columnCount",
    3,
    isRoundedNumberWithMinimumOfOne,
  );

  const [columnGap, setColumnGap] = useLocaleStorageState(
    "columnGap",
    100,
    isNumber,
  );
  ...
  const [previewScale, previewOffset] = getPreviewScaleAndOffset(
    boundingBox,
    previewSize,
  );
      ...
      <button
        ...
        onClick={async () => {
          ...
          for (const rect of sortedRects) {
            ...
            const rawBoundingBox = getBoundingBox(
              Object.values(rects).filter(isRect),
            );
...
function getPreviewScaleAndOffset(
  boundingBox: Rect,
  previewSize: Size,
): [number, Point] {
